...
const SUSHI_FACTORY_ADDRESS = toEthAddress(
    '0xc35DADB65012eC5796536bD9864eD8773aBc74C4'
)
const SUSHI_FACTORY_INIT_CODE = toFactoryInitCode(
    '0x34a26ac00aeedff252c746538fd4d97e26d9bc7e1d5a32b31c6e3cdea7c6147d'
)
...
function getPoolAddressByHash(factory, initCodeHash, addressA, addressB) {
    let [bufferA, bufferB] = _toAddressPairBuffer(addressA, addressB)

    if (!isEthAddress(bufferA) || !isEthAddress(bufferB)) {
        throw new Error('Invalid buffer ETH addresses')
    }

    if (
        Buffer.compare(bufferA, ADDRESS_ZERO) === 0 ||
        Buffer.compare(bufferB, ADDRESS_ZERO) === 0
    ) {
    ...
    let address = _getPoolAddressByHash(factory, initCodeHash, bufferA, bufferB)
...
function _getPoolAddressByHash(factory, initCodeHash, addressA, addressB) {
    ...
    let data = Buffer.concat([
        HEX_FF,
        toEthAddress(factory),
        salt,
        toFactoryInitCode(initCodeHash)
    ])
